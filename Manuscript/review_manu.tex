...
\todo{Samuli: Add citations to the introduction. Rewriting is also needed}
...
which directly calculate order parameters from heavy atom positions~\cite{??,vermeer07}.
...
simulations to reproduce the measured changes and then analyze the dynamical changes from the simulation trajectory~\cite{??}.
...
due to flexible dihedrals next to the double bonds~\cite{??}.

To ease the interpretation of the measured spin relaxation times Ferreira et al. recently introduced a model
free connection between effective correlation time and directly measurable spin relaxation rates~\cite{ferrerira15}.
...
correct order of magnitude~\cite{??}. Consequently, the dynamical visualizations of simulation trajectories 
...
The lipid rotational dynamics has been often interpreted by using the so called wobble in the cone model~\cite{??}.
...
experienced in acyl chain region may not reflect ot the headgroup~\cite{??} inficating weak coupling 
between these segments. This idea is also in line with one interpretation of recent field cycling experiments~\cite{??}.
In addition, the role of undulations in the relaxation data measured with low frequencies is under discussion~\cite{??}.
